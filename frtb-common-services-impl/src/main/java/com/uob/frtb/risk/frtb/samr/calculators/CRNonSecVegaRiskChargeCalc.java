...
import com.uob.frtb.risk.frtb.samr.config.CRNonSecVegaConfig;
...
@Component
@Scope("prototype")
public class CRNonSecVegaRiskChargeCalc extends BaseCRNonSecRiskChargeCalc {

	@Autowired
	private CRNonSecVegaConfig config;
    ...
    protected void calculateRiskCharge() {
		...
		debugCurrencyGrouping(bucketDeltas);
	...
	private Map<Integer, BucketLevelResults> calculateBucketLevelResults(Map<Integer, List<Group>> buckets) {
		...
		for (Map.Entry<Integer, List<Group>> me : buckets.entrySet()) {
			...
			log.debug("CR Non Sec Vega bucket level result : {}", r.toString());
		}
	...
	private void debugCurrencyGrouping(Map<GroupingKey, Group> bucketDeltas) {
		for (Group g : bucketDeltas.values()) {
	...
	void saveResults(PoResults results) {
		if (Double.compare(riskCharge, 0) == 0.0) {
			log.info("Credit Non Sec Vega - No results");
			return;
		}
		log.info("Credit Non Sec Vega - Saving results");
