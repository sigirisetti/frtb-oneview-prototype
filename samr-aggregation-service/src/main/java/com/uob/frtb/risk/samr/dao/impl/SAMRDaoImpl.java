...
import com.uob.frtb.risk.samr.results.SAMRResults;
import com.uob.frtb.risk.samr.results.SAMRValidationMessages;
...
public class SAMRDaoImpl extends HibernateDaoSupport implements SAMRDao {
	...
	private <T extends BaseTradeEntity> List<T> getFilteredSAMRData(Class<T> clazz, String workflowId,
		...
		String fn = "";
		if (HierarchyFilters.BOOK.equalsIgnoreCase(filter.getFilterName())) {
			fn = "book";
			hql += " and h.book in (:" + fn + ")";
		} else if (HierarchyFilters.DESK.equalsIgnoreCase(filter.getFilterName())) {
			fn = "desk";
			hql += " and h.desk in (:" + fn + ")";
		} else if (HierarchyFilters.LOCATION.equalsIgnoreCase(filter.getFilterName())) {
			fn = "location";
