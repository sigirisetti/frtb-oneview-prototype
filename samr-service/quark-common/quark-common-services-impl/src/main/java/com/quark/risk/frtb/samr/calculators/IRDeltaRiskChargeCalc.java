...
import com.quark.refdata.model.Tenor.TimeUnit;
import com.quark.risk.frtb.samr.config.IRDeltaConfig;
...
import com.quark.risk.frtb.samr.model.RiskWeight;
import com.quark.risk.frtb.samr.model.TradeSensitivity;
import com.quark.risk.frtb.samr.results.IRDeltaTradeLevelResults;
	...
	private void calculateSums(String currency, Sum ref, List<RiskWeight> riskWeights, Map<Integer, Delta> deltaMap) {
		...
		for (int i = 0; i < riskWeights.size(); i++) {
			...
			if (d != null) {
				...
				log.error("Delta not found for tenor : " + new Tenor(w.getTenorCode(), TimeUnit.D).toString());
			}
		}
