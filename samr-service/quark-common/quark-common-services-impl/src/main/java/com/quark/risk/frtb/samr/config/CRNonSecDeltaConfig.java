...
import static com.quark.risk.frtb.samr.config.ConfigUtil.populateWithDouble;
import static com.quark.risk.frtb.samr.config.ConfigUtil.populateWithIntegers;

@Getter
@Component
public class CRNonSecDeltaConfig {

	@Value("${samr.cr.non.sec.delta.tenor.buckets:0.5,1,3,5,10}")
	private String strCrNonSecDeltaTenorBuckets;

	@Value("${samr.cr.non.sec.delta.rw:0.005,0.01,0.05,0.03,0.03,0.02,0.015,0.04,0.03,0.04,0.12,0.07,0.085,0.055,0.05,0.120}")
	private String strRiskWeights;
	...
	private void init() {
		...
		populateWithIntegers(strCrNonSecDeltaIssuerBuckets, issuerBucketList);
		populateWithDouble(strCrNonSecDeltaTenorBuckets, tenorBucketList);
		populateWithDouble(strRiskWeights, riskWeights);
		...
		for (int i = 0; i < dim; i++) {
			for (int j = 0; j < dim; j++) {
				if (i == j) {
					...
					corr1.setEntry(i, j, crNonSecDeltaBucket1TenorCorr);
				}
			}
		}
